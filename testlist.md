...
- [x] **F. `parseCounts` の壊れた値の扱いを揃える**
      ...
      壊れたトレーラーを読んだときだけ効く。parseCounts のサイクルの
      Green で実施する
...
- [x] formatCounts の出力を往復で戻せる
- [x] 空文字
- [x] 名前に `=` を含む（`lastIndexOf` で区切る挙動）
- [x] 区切りが無い／先頭が `=` の壊れた要素は捨てる
- [x] 数値にならない値は 0 になる
- [x] 非有限な値は 0 になる
- [x] 負の値は 0 になる

`Number(...) || 0` は NaN しか 0 に潰さず、`Infinity` と負の値は素通りしていた。
前者は差分側（`diff` の `Math.max(..., 0)`）で 0 に落ちてその名前の増分が
まるごと消え、後者は基準として引かれて実際より多い数字が残る。どちらも
...
- [x] 増えた名前は基準を差し引いた増分になる
- [x] 基準に無い名前は累計がそのまま増分になる
- [x] 基準の値が 0 の名前は累計がそのまま増分になる
      （parseCounts が壊れた値を 0 に寄せたときに通る経路）
- [x] 変化の無い名前は落ちる
- [x] 減っている名前は落ちる（diff が 0 のため）
- [x] 基準にだけある名前は結果に現れない（空の total でも兼ねる）

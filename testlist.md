...
`Number(...) || 0` は NaN を 0 に潰すが、`Infinity` と負の値は素通りする。
前者は差分側（`diff` の `Math.max(..., 0)`）で 0 に落ちて、その名前の増分が
まるごと消える。後者は基準として引かれて実際より多い数字が残る。どちらも
「値が壊れていた要素は捨てる」という関数上のコメントの方針から漏れているため、
このサイクルの Green で `Number.isFinite(parsed) && 0 <= parsed` の判定を
足し、0 に寄せる（設計変更 F）。

...
- 対象: round6
- 完了: 1. テスト項目の再検討 → 2. Red
- 次: 3. Green（設計変更 A・B を含む）
...
round6 の責務ではないことを、非有限な値を素通しすることで表現する。
...
負の totalCost は round6 も diff も素通りする。diff の `Math.max(..., 0)` は
